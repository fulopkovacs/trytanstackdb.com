  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-fit max-w-80 flex flex-col gap-2">
        ...
        <p className="text-xs text-muted-foreground">
          This is because currently they are handled locally by a service worker
          and this implementation comes with this limitation.
        </p>
        ...
        {networkLatencyOptions.map((option) => (
          <div key={option.value} className="">
            <Button
              key={option.value}
              variant={selectedLatency === option.value ? "secondary" : "ghost"}
              size="sm"
              onClick={() => {
