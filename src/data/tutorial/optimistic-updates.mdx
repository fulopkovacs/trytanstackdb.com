...
2. When the HTTP request fails in the background (because all project names
   ...
   - A new `GET` request to `/api/projects` syncs the local state with the remote database. You can watch this happen in the <OpenAPIRequestsPanelLink>_API Requests_ panel</OpenAPIRequestsPanelLink>
